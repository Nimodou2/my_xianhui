 ...
 */
public class YeJiZhuChengFragment3 extends Fragment {
    ...
    private PieData getPieData(float range,List<List<Card>> data) {
        //Log.e("data2,size:",data.size()+"");
        ArrayList<PieEntry> yValues = new ArrayList<PieEntry>();  //yVals用来表示封装每个饼块的实际数据
        ArrayList<String> xValues = new ArrayList<String>();  //xVals用来表示每个饼块上的内容
        for (int i = 0; i < data.get(2).size(); i++) {
            xValues.add(data.get(2).get(i).getFullname());  //饼块上显示成Quarterly1, Quarterly2, Quarterly3, Quarterly4
            yValues.add(new PieEntry(data.get(2).get(i).getAmounts(), data.get(2).get(i).getFullname()));
        }
